...
struct ElementView: View {
    ...
    /// Check if this element should be hidden (internal Claude Code messages)
    private var shouldHide: Bool {
        if case .userInput(let content) = element.content {
            return content.text.hasPrefix("Caveat: The messages below were generated by the user")
        }
        return false
    }
...
struct UserInputElementView: View {
    ...
    var body: some View {
        ...
        if segments.count == 1, case .text(let text) = segments[0], text == content.text {
